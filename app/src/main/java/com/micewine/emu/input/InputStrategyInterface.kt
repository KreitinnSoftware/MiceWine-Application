...
interface InputStrategyInterface {
     ...
     */
    fun onPressAndHold(button: Int): Boolean
    ...
    class NullInputStrategy : InputStrategyInterface {
        override fun onTap(button: Int) {}
        override fun onPressAndHold(button: Int): Boolean {
            return false
        }
     ...
     */
    class SimulatedTouchInputStrategy(
        renderData: RenderData, injector: InputEventSender?, context: Context?
    ) : InputStrategyInterface {
        private val mRenderData: RenderData
        private val mInjector: InputEventSender

        /**
         * The maximum distance, in pixels, between two points in order for them to be considered a
         * double tap gesture.
         */
        private val mDoubleTapSlopSquareInPx: Int
        ...
        init {
            ...
            var scaledDoubleTapSlopInPx = config.scaledDoubleTapSlop
            scaledDoubleTapSlopInPx =
                (scaledDoubleTapSlopInPx * DOUBLE_TAP_SLOP_SCALE_FACTOR).toInt()
            mDoubleTapSlopSquareInPx = scaledDoubleTapSlopInPx * scaledDoubleTapSlopInPx
        }

        override fun onTap(button: Int) {
            val currentTapPoint = mRenderData.cursorPosition
            if (button == InputStub.BUTTON_LEFT) {
                ...
                val tapInterval = SystemClock.uptimeMillis() - mLastTapTimeInMs
        ...
        override fun onMotionEvent(event: MotionEvent) {
            if (event.actionMasked == MotionEvent.ACTION_UP && mHeldButton != InputStub.BUTTON_UNDEFINED) {
                mInjector.sendMouseUp(mHeldButton, false)
                mHeldButton = InputStub.BUTTON_UNDEFINED
            }
        }

        private fun isDoubleTap(currentX: Float, currentY: Float, tapInterval: Long): Boolean {
            ...
            //
            val currentValues =
                floatArrayOf(currentX * mRenderData.scale.x, currentY * mRenderData.scale.y)
            val previousValues = floatArrayOf(
                mLastTapPoint!!.x * mRenderData.scale.x,
                mLastTapPoint!!.y * mRenderData.scale.y
            )
            val deltaX = (currentValues[0] - previousValues[0]).toInt()
            val deltaY = (currentValues[1] - previousValues[1]).toInt()
        ...
        companion object {
             ...
             */
            private const val DOUBLE_TAP_SLOP_SCALE_FACTOR = 0.25f
        }
    }
     ...
     */
    class TrackpadInputStrategy : InputStrategyInterface {
        private var mInjector: InputEventSender? = null
